> Pi 可以创建 TUI 组件。让它根据你的使用场景构建一个即可。

# TUI 组件

扩展和自定义工具可以渲染自定义 TUI 组件，用于交互式用户界面。本页介绍组件系统和可用的构建模块。

**源代码：** [`@earendil-works/pi-tui`](https://github.com/earendil-works/pi/tree/main/packages/tui)
...
| 方法 | 说明 |
|--------|-------------|
| `render(width)` | 返回字符串数组（每行一个字符串）。每行都**不得超过 `width`**。 |
| `handleInput?(data)` | 组件获得焦点时接收键盘输入。 |
| `handleMouse?(event)` | 在全屏模式下接收规范化的指针输入。 |
| `wantsKeyRelease?` | 如果为 true，组件会接收按键释放事件（Kitty 协议）。默认值：false。 |
| `invalidate()` | 清除缓存的渲染状态。在主题变更时调用。 |

TUI 会在每个已渲染行的末尾追加完整的 SGR 重置和 OSC 8 重置。样式不会跨行延续。如果输出带样式的多行文本，请在每行重新应用样式，或使用 `wrapTextWithAnsi()`，以便为每个换行后的行保留样式。

## 可聚焦接口（IME 支持）

显示文本光标并且需要 IME（输入法编辑器）支持的组件应实现 `Focusable` 接口：
...
光标默认保持隐藏。这样既能保留模拟光标的渲染，又能为使用隐藏光标追踪 IME 候选窗口的终端定位硬件光标。某些终端需要可见的硬件光标才能定位 IME；可通过渲染器的 `showHardwareCursor` 构造函数参数或 `setShowHardwareCursor(true)` 启用。Pi 还会在创建渲染器之前，将 `PI_HARDWARE_CURSOR=1` 映射到此设置。内置的 `Editor` 和 `Input` 组件已实现此接口。
...
class SearchDialog extends Container implements Focusable {
  ...
  // Focusable 实现——将焦点传递给子输入框，以便定位 IME 光标
...
如果不进行这种传递，使用 IME（中文、日文、韩文等）输入时，候选窗口会显示在屏幕上的错误位置。
...
浮层会在现有内容上方渲染组件，而不清除屏幕。将 `{ overlay: true }` 传给 `ctx.ui.custom()`：
...
const result = await ctx.ui.custom<string | null>(
  (tui, theme, keybindings, done) => new SidePanel({ onClose: done }),
  {
    ...
    onHandle: (handle) => {
      // handle.focus()——聚焦此浮层并将其置于视觉最前方
      // handle.unfocus()——将输入释放给普通回退目标
...
获得焦点且可见的浮层在临时出现的非浮层 UI 期间仍保有输入所有权。如果某个浮层打开了另一个未设置 `{ overlay: true }` 的 `ctx.ui.custom()` 组件，则该替代 UI 在活动期间接收输入；关闭后，已聚焦的浮层可以重新取得输入。

当可见浮层应停止拥有输入，并让 TUI 回退到另一个可见且可捕获输入的浮层或先前的焦点目标时，请使用 `handle.unfocus()`。当浮层保持可见、但需要由指定组件接收输入时，请使用 `handle.unfocus({ target })`。传入 `{ target: null }` 会有意保持无组件聚焦，直到再次设置焦点。

### 浮层生命周期

浮层组件关闭时会被释放。不要复用引用——请创建新实例：
...
全屏模式会将规范化的按下、释放、单击、移动、拖动和滚轮事件路由到组件和浮层。返回 `{ handled: true }` 可抑制默认行为；返回 `capture: true` 可保留拖动/释放事件的所有权；返回 `focus: true` 可请求键盘焦点；当悬停或释放操作明显改变组件时，返回 `render: true`。按下、单击、拖动和滚轮默认触发渲染；无操作的移动/释放事件不会触发。
...
未处理的滚轮输入会滚动最近的 `ScrollView`；未处理的主按钮拖动会保留对话记录选择。OSC 8 链接优先于父级点击区域。`Input`、`Editor`、`SelectList` 和 `SettingsList` 包含全屏鼠标行为。常规模式不捕获鼠标输入，因为终端需要管理自身的回滚缓冲区。

## 行宽

**重要：** `render()` 返回的每一行都不得超过 `width` 参数。
...
- `truncateToWidth(str, width, ellipsis?)`——截断文本，可选添加省略号
...
主题变更时，TUI 会在所有组件上调用 `invalidate()` 来清除其缓存。组件必须正确实现 `invalidate()`，以确保主题变更生效。

### 问题

如果组件预先通过 `theme.fg()`、`theme.bg()` 等方式将主题颜色固化到字符串中并进行缓存，则缓存的字符串会包含旧主题的 ANSI 转义代码。如果组件还单独存储了带主题样式的内容，仅清除渲染缓存并不足够。
...
class BadComponent extends Container {
  ...
  constructor(message: string, theme: Theme) {
    super();
    // 将预先固化的主题颜色存储在 Text 组件中
    this.content = new Text(theme.fg("accent", message), 1, 0);
    this.addChild(this.content);
  }
  // 未重写 invalidate——父类的 invalidate 只会清除
  // 子组件的渲染缓存，而不会清除预先固化的内容
}
```
...
1. **预先固化主题颜色**——使用 `theme.fg()` 或 `theme.bg()` 创建带样式的字符串并存储在子组件中
2. **语法高亮**——使用 `highlightCode()`，它会应用基于主题的语法颜色
3. **复杂布局**——构建嵌入主题颜色的子组件树

以下情况不需要此模式：

1. **使用主题回调**——传入类似 `(text) => theme.fg("accent", text)` 的函数，在渲染期间调用
2. **简单容器**——仅组合其他组件，不添加带主题样式的内容
3. **无状态渲染**——在每次 `render()` 调用中重新计算带主题样式的输出（不缓存）
...
用于让用户从选项列表中进行选择。使用 `@earendil-works/pi-tui` 中的 `SelectList`，并通过 `DynamicBorder` 添加边框。
...
用于耗时且应允许取消的操作。`BorderedLoader` 会显示旋转指示器，并处理 Escape 键以取消操作。
...
用于切换多项设置。使用 `@earendil-works/pi-tui` 中的 `SettingsList` 和 `getSettingsListTheme()`。
...
这只影响正常流式输出时的工作指示器。上下文压缩和重试加载器仍使用其内置样式。自定义帧会逐字渲染，因此扩展必须按需自行添加颜色。

**示例：** [working-indicator.ts](../examples/extensions/working-indicator.ts)

### 模式 5：编辑器上方/下方的小组件

在输入编辑器上方或下方显示持久内容。适合待办事项列表、进度信息。
...
替换页脚。`footerData` 会公开扩展无法通过其他方式访问的数据。
...
使用自定义实现替换主输入编辑器。适用于模态编辑（Vim）、不同的键绑定（Emacs）或专门的输入处理。
...
class VimEditor extends CustomEditor {
  ...
  render(width: number): string[] {
    ...
    // 在下边框添加模式指示器（使用 truncateToWidth 进行 ANSI 安全截断）
    if (lines.length > 0) {
...
- 对未自行处理的按键**调用 `super.handleInput(data)`**
- **状态旋转指示器**：自定义编辑器默认保留独立状态行。将 `{ embedWorkingStatus: true }` 作为 `CustomEditor` 构造函数的第四个参数传入，可将工作、上下文压缩、分支摘要和重试旋转指示器嵌入编辑器边框中。
...
2. **始终为 DynamicBorder 的颜色参数标注类型**——应写成 `(s: string) => theme.fg("accent", s)`，而不是 `(s) => theme.fg("accent", s)`。

3. **状态变更后调用 tui.requestRender()**——在 `handleInput` 中更新状态后，调用 `tui.requestRender()`。

4. **返回包含三个方法的对象**——自定义组件需要 `{ render, invalidate, handleInput }`。

5. **使用现有组件**——`SelectList`、`SettingsList`、`BorderedLoader` 可覆盖 90% 的使用场景。不要重新构建它们。
